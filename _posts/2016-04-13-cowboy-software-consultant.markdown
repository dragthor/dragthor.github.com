 ...
 * Deletes, skips, disregards, and [eats unit tests](/consultants-ate-my-unit-tests/) for breakfast.
 * Tests in production.
 * Tests trivial things to make it seem like they are testing and have decent code coverage.  You know, testing for testing's sake.

## 3. Cowboy Source Control
 * Does not use client's source control (or use it correctly).  What's a branch?
 * Works in a disconnected silo, then delivers a mess of disorganized non-synced code.
 ...
 * Code formatting not in sync with the team (3 vs. 4 tab indentation, `{ }` placement).  Thus when you view the changeset diff, pretty much everything has changed.  The consultant's change is lost in a sea of changes.
 ...
 * Rearranges code for the sake of rearranging code.  This "looks" like work, but it is not refactoring.
